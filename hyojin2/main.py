...
def train():
    ...
    start_epoch = 0
    best_valid_acc = 0
    # Optimizer Momentum, Learning_rate 조절
    one_cycle = OneCyclePolicy(optimizer, num_steps=const.EPOCHS, momentum_range=(0.85, 0.95), lr_range=(1e-4, 1e-1))
    ...
    for epoch in main_pbar:
        ...
        valid_acc = 0
        correct_sum = 0
        valid_pbar = tqdm(enumerate(val_loader), total=num_valid, desc="Validation", position=1, leave=False)
        with torch.no_grad():
            for i, (x1, x2, y) in valid_pbar:
                ...
                valid_acc = correct_sum / const.NUM_VAL
...
def test():
    ...
    test_acc = correct_sum / const.NUM_TEST
